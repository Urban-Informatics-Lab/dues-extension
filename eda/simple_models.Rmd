...
e_actual %>% 
  ...
  left_join(e_sim, by = c("apn", "date_time")) %>% 
  mutate(
    kwh_actual = if_else(kwh_actual < 1, 1, kwh_actual),
    percent_diff = 100 *  abs((kwh_actual - kwh) / kwh_actual)
  ) %>% 
  drop_na(percent_diff) %>% 
  group_by(apn) %>% 
  summarize(mean = mean(percent_diff)) %>% 
  arrange(mean)
```
